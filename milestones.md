# Milestone 2
## Video
https://www.youtube.com/watch?v=1KWM57SjoOk&feature=youtu.be

## New Design Docs
### Client Design Doc
https://docs.google.com/document/d/1uuWtZ_WlORaCHqvfoVyy2kddN_-l-5EShkVs6VJQvh0/edit?usp=sharing

## Meeting Notes
https://docs.google.com/document/d/1ptGUhw1XonetMxNX3KzEdqZCvcjl_PuefJGZ4JPfokA/edit?usp=sharing

## PRs
### Heping (Send unreceived messages to client)
https://github.com/ECS153/final-project-group-1/pull/7

### Lawrence (Client send and receive threads, added decrpytion)
https://github.com/ECS153/final-project-group-1/pull/5

### Darian (Collect message sent from client, write into database)
https://github.com/ECS153/final-project-group-1/pull/6

### Zach (Transaction upload to Etherium test net)
https://github.com/ECS153/final-project-group-1/pull/8



# Milestone 1
## Video
https://www.youtube.com/watch?v=OGG8BhOcmUQ&feature=youtu.be

## Design Docs
### Database Design Doc
https://docs.google.com/document/d/16OjasaJ715R0NuA8JiZffXzA-v3KYNQigUNfPRnD4XQ/edit?usp=sharing

## Meeting Notes
https://docs.google.com/document/d/1ptGUhw1XonetMxNX3KzEdqZCvcjl_PuefJGZ4JPfokA/edit?usp=sharing

## PRs
### Heping (hash generator implementation)
https://github.com/ECS153/final-project-group-1/pull/4

### Lawrence (basic client implementation, with encryption added)
https://github.com/ECS153/final-project-group-1/pull/3

### Zach (basic server, database creation, Public/Private keys creation)
https://github.com/ECS153/final-project-group-1/pull/2

### Zach (defined API def for hash generator, Block chain, Merkle Tree
https://github.com/ECS153/final-project-group-1/pull/1
